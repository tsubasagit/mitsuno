...
function getApiKey() {
    // テストモードの場合はサンプルキーを返す
    if (isTestMode()) {
        return 'sk-test-sample-key-for-development';
    }
...
window.addEventListener('DOMContentLoaded', () => {
    // テストモードの場合はAPIキー入力モーダルを表示しない
    if (isTestMode()) {
        console.log('テストモードが有効です。APIキー入力はスキップされます。');
        return;
    }
    
    // プロキシサーバーを使用する場合はAPIキー入力不要
    if (useProxyServer()) {
        console.log('プロキシサーバーが設定されています。APIキー入力は不要です。');
        return;
    }
    
    // DIFY APIを使用する場合はAPIキー入力不要
    if (useDify()) {
        console.log('DIFY APIが設定されています。APIキー入力は不要です。');
        return;
    }
